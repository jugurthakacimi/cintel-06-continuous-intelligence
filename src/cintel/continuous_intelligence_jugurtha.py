...
DATA_FILE: Final[Path] = DATA_DIR / "transaction_metrics_jugurtha.csv"
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "transaction_assessment_jugurtha.csv"
...
# Thresholds — flag_rate > 1% or avg amount spikes above $160
MAX_FLAG_RATE: Final[float] = 0.01
MAX_AVG_AMOUNT: Final[float] = 160.0
...
def main() -> None:
    ...
    df = df.with_columns(
        [
            (pl.col("flagged_transactions") / pl.col("transactions")).alias(
                "flag_rate"
            ),
            (pl.col("total_transaction_amount_usd") / pl.col("transactions")).alias(
                "avg_amount_usd"
            ),
        ]
    )
    ...
    anomalies_df = df.filter(
        (pl.col("flag_rate") > MAX_FLAG_RATE)
        | (pl.col("avg_amount_usd") > MAX_AVG_AMOUNT)
    )
    LOG.info(
        f"STEP 3. Using thresholds: MAX_FLAG_RATE={MAX_FLAG_RATE}, "
        f"MAX_AVG_AMOUNT={MAX_AVG_AMOUNT}"
    )
    ...
    summary_df = summary_df.with_columns(
        pl.when(
            (pl.col("avg_flag_rate") > MAX_FLAG_RATE)
            | (pl.col("avg_amount_usd") > MAX_AVG_AMOUNT)
        )
